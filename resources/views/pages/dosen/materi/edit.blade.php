        ...
        <form id="kt_ecommerce_add_category_form" class="form d-flex flex-column flex-lg-row" enctype="multipart/form-data">
            ...
            <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                <div class="card card-flush py-4">
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div>
                            ...
                            <textarea name="deskripsi" class="form-control mb-2" placeholder="Deskripsi Materi" required rows="5">{{ old('deskripsi', $materi->deskripsi) }}</textarea>
                            <div class="text-muted fs-7">
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="mb-10">
                            ...
                            <div id="quill_isi_materi" class="min-h-400px mb-2">
                                {!! old('isi_materi', $materi->isi_materi) !!}
                            </div>
                        ...
                        @if($list_file && $list_file->count() > 0)
                        <div class="mb-10">
                            <label class="form-label">{{ __('class.existing_files') }}</label>
                            <div class="border border-gray-300 border-dashed rounded p-6">
                                @foreach($list_file as $file)
                                <div class="d-flex align-items-center justify-content-between mb-3">
                                    <div class="d-flex align-items-center">
                                        <i class="ki-duotone ki-file text-primary fs-2x me-3">
                                            <span class="path1"></span>
                                            <span class="path2"></span>
                                        </i>
                                        <div>
                                            <a href="{{ asset('storage/' . $file->file) }}" target="_blank" class="text-gray-800 text-hover-primary fw-bold">
                                                {{ basename($file->file) }}
                                            </a>
                                            <div class="text-muted fs-7">
                                                {{ __('class.click_to_download') }}
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('dosen.kelas.show', $kode_kelas) }}" id="kt_ecommerce_add_product_cancel"
                        class="btn btn-light me-5">{{ __('class.cancel') }}</a>
                    <button type="submit" id="submit" class="btn btn-primary">
                        <span class="indicator-label">{{ __('class.update') }}</span>
                    </button>
                </div>
            </div>
        </form>
...
@section('scripts')
    <script>
        ...
        // Set initial content
        quill.root.innerHTML = `{!! addslashes($materi->isi_materi) !!}`;
    ...
    <script>
        var myDropzone = new Dropzone("#materi_file", {
            url: "{{ route('dosen.materi.update', [$kode_kelas, $materi->id]) }}",
            ...
            init: function() {
                ...
                submitButton.addEventListener("click", function(e) {
                    ...
                    // If no files to upload, submit form directly
                ...
                this.on("sending", function(file, xhr, formData) {
                    ...
                    var materi_id = document.querySelector('input[name="materi_id"]').value;
        ...
        function submitFormWithoutFiles() {
            ...
            formData.append('kode_kelas', document.querySelector('input[name="kode_kelas"]').value);
            formData.append('materi_id', document.querySelector('input[name="materi_id"]').value);
            ...
            formData.append('isi_materi', document.querySelector('input[name="isi_materi"]').value);
            formData.append('status', document.querySelector('select[name="status"]').value);

            fetch('{{ route("dosen.materi.update", [$kode_kelas, $materi->id]) }}', {
                method: 'POST',
                body: formData
            })
            ...
            .catch(error => {
                ...
                Swal.fire({
                    text: 'Terjadi kesalahan saat mengupdate materi',
                    icon: "error",
                    buttonsStyling: false,
                    confirmButtonText: "Ok, got it!",
                    customClass: {
        ...
        function deleteFile(fileId) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route("dosen.materi.deleteFile", ":fileId") }}'.replace(':fileId', fileId), {
                        method: 'DELETE',
                        headers: {
